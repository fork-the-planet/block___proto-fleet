...
const getOrgScopeAssignmentForUser = `-- name: GetOrgScopeAssignmentForUser :one
SELECT
...
type GetOrgScopeAssignmentForUserParams struct {
	UserID         int64
	OrganizationID int64
}

type GetOrgScopeAssignmentForUserRow struct {
	AssignmentID   int64
	RoleID         int64
	RoleBuiltinKey sql.NullString
}
...
//     commits.
func (q *Queries) GetOrgScopeAssignmentForUser(ctx context.Context, arg GetOrgScopeAssignmentForUserParams) (GetOrgScopeAssignmentForUserRow, error) {
	row := q.queryRow(ctx, q.getOrgScopeAssignmentForUserStmt, getOrgScopeAssignmentForUser, arg.UserID, arg.OrganizationID)
	var i GetOrgScopeAssignmentForUserRow
	err := row.Scan(&i.AssignmentID, &i.RoleID, &i.RoleBuiltinKey)
	return i, err
}
...
const lockAndCountOrgScopeSuperAdmins = `-- name: LockAndCountOrgScopeSuperAdmins :one
...
func (q *Queries) LockAndCountOrgScopeSuperAdmins(ctx context.Context, organizationID int64) (int64, error) {
	row := q.queryRow(ctx, q.lockAndCountOrgScopeSuperAdminsStmt, lockAndCountOrgScopeSuperAdmins, organizationID)
	var super_admin_count int64
	err := row.Scan(&super_admin_count)
	return super_admin_count, err
}
