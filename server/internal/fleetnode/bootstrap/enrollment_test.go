...
func TestRegister_HappyPath(t *testing.T) {
	...
	assert.Equal(t, 32, len(fake.encryptionPub))
	assert.Equal(t, 32*2, len(result.State.EncryptionPublicKeyHex))
	assert.Equal(t, 32*2, len(result.State.EncryptionPrivateKeyHex))
